...
def load_data(filename):
    ...
    """
    print("loading data for {}".format(filename))
    D = []
    with open(filename, "r", encoding='utf-8') as f:
        for l in f:
            ...
            D.append({
                'text': l['text'],
                'spo_list': [(spo[0], spo[1], spo[2]) for spo in l['spo']]
            })
    return D


# 加载数据集
train_data = load_data('./data/alias_train.json')
valid_data = load_data('./data/alias_test.json')
...
class Evaluator(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if f1 >= self.best_val_f1:
            self.best_val_f1 = f1
            train_model.save_weights('./models/alias_best_model.weights')
